...
export function InvoiceSummaryCard({ invoice }: InvoiceSummaryCardProps) {
  ...
  return (
    <article className="rounded-2xl border border-brand-800/65 bg-[radial-gradient(circle_at_top_left,rgba(49,214,103,0.12),transparent_46%),linear-gradient(135deg,rgba(12,25,20,0.94),rgba(12,15,17,0.98))] p-3.5 shadow-lg shadow-black/25">
      <div className="flex items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <div className="flex min-w-0 items-center gap-2">
            <h2 className="truncate text-base font-semibold leading-none text-app-text">
              {invoice.cardName}
            </h2>
            <ChevronDown aria-hidden="true" className="h-4 w-4 shrink-0 text-app-text" />
            <Badge
              variant={statusVariant[invoice.status]}
              className="ml-0.5 px-1.5 py-0 text-[0.6rem]"
            >
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <div className="min-w-0">
          <p className="text-[0.78rem] leading-5 text-app-muted">Vencimento</p>
          <p className="mt-0.5 truncate text-[1.12rem] font-bold leading-tight text-brand-400">
            {invoice.dueDate}
          </p>
        </div>

        <div className="min-w-0 border-l border-app-border pl-3">
          <p className="text-[0.78rem] leading-5 text-app-muted">
      ...
      <div className="mt-3.5 border-t border-app-border pt-3.5">
        <div className="flex items-end justify-between gap-4">
          <div className="min-w-0">
            <p className="text-[0.78rem] leading-5 text-app-muted">Total da fatura</p>
            <strong className="mt-1 block truncate text-[1.72rem] font-bold leading-tight text-app-text">
              {moneyFormatter.format(invoice.total)}
            </strong>
          </div>

          <div className="shrink-0 text-right">
            <p className="text-[0.78rem] leading-5 text-app-muted">Limite</p>
            <p className="mt-1 text-[0.9rem] font-semibold leading-tight text-brand-400">
        ...
        <p className="mt-2 text-[0.72rem] font-medium text-brand-400">
          {progress}% utilizado
        </p>
      </div>
      ...
      <button
        type="button"
        className={cn(
          'mt-4 flex h-10 w-full items-center gap-2 rounded-xl bg-brand-950/55 px-3 text-left text-[0.82rem] font-semibold text-app-text transition-colors',
